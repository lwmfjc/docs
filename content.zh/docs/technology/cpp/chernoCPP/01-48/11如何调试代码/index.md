...
箭头在此处，表示该行代码即将执行（未执行），所以此时a为任意可能值(未初始化的内存)  
...
16^2 x 1 + 16^0 x1 =256 + 1 =257 

很明显，这里用的是小端法， ~~这里==1个==int占用4字节~~ 在内存中按最低有效字节到最高有效字节(01,01,00,00)的顺序在内存中排列  
...
-  ~~这里一个char占用一个字节~~ 接上，地址0x00007ff7d45aac10值为Hello，即H-72-0x48，e-101-0x65，l-108-0x6C，l-108-0x6C，o-111-0x6F
