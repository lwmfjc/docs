...
> 1. 摄像机一开始是看像原点的，后来又看向了其他地方。而这会导致相对x轴而言，形成了偏航角，相对y轴而言形成了俯仰角。
> 2. 这里取了相机和原来的原点上形成的向量的单位向量进行的计算（长度为1）

> 1. 鼠标水平移动 $\rightarrow$ 改变 偏航角 (Yaw) $\rightarrow$ 绕 Y 轴 旋转
> 2. 鼠标垂直移动 $\rightarrow$ 改变 俯仰角 (Pitch) $\rightarrow$ 绕 X 轴 旋转


欧拉角是三个可以表示三维空间中任意旋转的数值，由莱昂哈德·欧拉在 18 世纪左右==定义== ~~注意，这里定义 俯仰角是绕着x轴产生的；偏航角绕着y轴产生的~~ 。这三个欧拉角分别是： _俯仰角_ ~~pitch~~  、 _偏航角_  ~~yaw~~ 和 _滚转角_  ~~roll~~ 。下图直观地展示了它们的含义：  

![](img/ly-20260319080440675.png)

俯仰角表示我们向上或向下看的角度，如图一所示。图二显示的是偏航角，表示我们向左或向右看的角度。横滚角表示我们*向右滚动*的角度，主要用于航天相机。每个欧拉角都用一个单独的值表示，结合这三个角度，我们可以计算出三维空间中的任何旋转矢量。

==对于我们的相机系统，我们只关心*偏航角*和*俯仰角*的值，因此这里不讨论滚转角的值。==给定一个俯仰角和偏航角值，我们可以将它们==转换为一个表示新方向向量的三维向量==。==将偏航角和俯仰角值转换为方向向量==的过程需要用到一些三角函数知识。我们从一个基本情况开始：
...
这样就解决了*如何从偏航角值获取三维方向向量*的问题，但俯仰角也需要考虑。现在让我们假设自己位于 `xz` 平面上，来看一下 `y` 轴方向：
...
这为我们==提供了一个公式，可以将偏航角和俯仰角值转换为三维方向向量==，我们可以利用该向量环顾四周。
...
> 1. 这里说的是上面那个公式，如果yaw=0，pitch=0的话，direction.x = cos(glm::radians(yaw)) * cos(glm::radians(pitch))=1;direction.y = sin(glm::radians(pitch))=0; direction.z = sin(glm::radians(yaw)) * cos(glm::radians(pitch))=0;即相机视线，默认指向的是正x轴方向。
> 2. opengl中我们应该默认指向负z轴方向，即(0,0,-1)。所以这里让yaw=-90 
...
偏航角和俯仰角的值是通过鼠标（或控制器/操纵杆）的移动获得的，其中==鼠标的水平移动影响偏航角，垂直移动影响俯仰角==。其原理是存储上一帧的鼠标位置，并在当前帧中计算鼠标位置的变化量。水平或垂直方向的变化越大，俯仰角或偏航角的更新幅度就越大，因此摄像机的移动幅度也应该越大。  

> 1. 鼠标水平移动 $\rightarrow$ 改变 偏航角 (Yaw) $\rightarrow$ 绕 Y 轴 旋转
> 2. 鼠标垂直移动 $\rightarrow$ 改变 俯仰角 (Pitch) $\rightarrow$ 绕 X 轴 旋转

首先，我们要告诉 GLFW 隐藏并捕获光标。捕获光标意味着，==一旦应用程序获得焦点，鼠标光标就会保持在窗口中心（除非应用程序失去焦点或退出）==。我们可以通过一个简单的配置调用来实现这一点 ~~在 3D 游戏中，你的“准星”通常固定在屏幕正中心，鼠标的移动应该直接对应视角的转动，而不是去移动一个 2D 的图标~~ ：  
...
现在运行这段代码，你会注意到，当鼠标光标首次聚焦到窗口时，相机会发生一次大幅度的突然跳动。造成这种突然跳动的原因是，==当光标进入窗口时，鼠标回调函数会被立即调用，并传入鼠标进入屏幕时的 xpos 和 ypos 坐标值==。这个位置通常距离屏幕中心很远，导致较大的偏移量，从而产生较大的移动跳动。我们可以通过==定义一个全局 `bool` 变量来规避这个问题，该变量用于检查是否是第一次接收到鼠标输入。如果是第一次，则*将初始鼠标位置更新为新的 xpos 和 `ypos` 值==*。之后，鼠标的移动将使用新输入的鼠标位置坐标来计算偏移量：
