...
![](img/22c648aff4df2679268bbc9dc2b9a25a_MD5.jpg)

但如果马上重启 MySQL 实例，重启后这个表的 AUTO_INCREMENT 就会变成 1。﻿ 也就是说，MySQL 重启可能会修改一个表的 AUTO_INCREMENT 的值。

![](img/9c69db2d3ae0c6a026088bb1bd7e9414_MD5.jpg)

![](img/ac16f3e5851d3cbfec42806a3ecab5e3_MD5.jpg)
...
![](img/0af9bd7d00ccc6a9be029546205045ca_MD5.jpg)

这时我再执行一条插入 `(null,1,1)` 的命令，很显然会报错 `Duplicate entry`，因为我们设置了一个唯一索引字段 `a`：

![](img/80c4b3ab6d80af035a2b1b96f4141b05_MD5.jpg)
...
![](img/9127620c33e6937696106d2b1c471aef_MD5.jpg)

我们先插入一行数据 `(null, 2, 2)`，也就是 (3, 2, 2) 嘛，并且 AUTO_INCREMENT 变为 4：

![](img/122bfc34cac833c5023ea212df54ad6c_MD5.jpg)

再去执行这样一段 SQL：

![](img/3a2b28d18ddebca639f07cf9a0d527cf_MD5.jpg)

虽然我们插入了一条 (null, 3, 3) 记录，但是使用 rollback 进行回滚了，所以数据库中是没有这条记录的：

![](img/3bf50f797ad120a270d1a99d7c9cb6e0_MD5.jpg)

在这种事务回滚的情况下，自增值并没有同样发生回滚！如下图所示，自增值仍然固执地从 4 增加到了 5：

![](img/45b3f0071b27a5999ffc309f7cb817fb_MD5.jpg)

所以这时候我们再去插入一条数据（null, 3, 3）的时候，主键 id 就会被自动赋为 `5` 了：

![](img/05ce084eab61bf3abaa600820b44a71c_MD5.jpg)
...
![](img/58b954b56c22574d8a5ac8cd386a0a4c_MD5.jpg)
...
![](img/0706af9c0314a71f18be5eca43eb77db_MD5.jpg)

我们创建一个和当前表 `test_pk` 有相同结构定义的表 `test_pk2`：

![](img/580ee6d12f82000e4adf147ccb0173ce_MD5.jpg)

然后使用 `insert...select` 往 `teset_pk2` 表中批量插入数据：

![](img/f2d6fa650767d4d20dfe4195bd91f5e2_MD5.jpg)

可以看到，成功导入了数据。

再来看下 `test_pk2` 的自增值是多少：

![](img/f83ceaee7f6d4406abd8621dc1472891_MD5.jpg)
...
![](img/177533dd8b257550dd393932d3269f9f_MD5.jpg)
