...
JDK1.8之后：  
![Java 运行时数据区域（JDK1.8 之后）](img/ly-20241212141952951.png)
...
- 动态链接  主要服务**一个方法需要调用其他方法**的场景。
  ...
  ![img](img/ly-20241212141953531.png)
...
- 总结，程序运行中栈可能出现的两种错误
  ...
  > ![img](img/ly-20241212141953675.png)
...
- JDK7及JDK7之前，堆内存被分为三部分
  ...
  ![hotspot-heap-structure](img/ly-20241212141953830.png)
...
- 大部分情况，对象都会首先在 Eden 区域分配，在一次新生代垃圾回收后，如果对象还存活，则会进入 S0 或者 S1，并且对象的年龄还会加 1(**Eden 区->Survivor 区后对象的初始年龄变为 1**)，当它的年龄增加到一定程度（默认为 15 岁），就会被晋升到老年代中。
  ...
  > ![ly-20241212141953985](img/ly-20241212141953985.png)
  >
  > ---
  >
  > 代码如下：  
...
- **方法区和永久代以及元空间有什么关系呢？**
  ...
  ![HotSpot 虚拟机方法区的两种实现](img/ly-20241212141954137.png)

- 为什么将永久代（PermGen）替换成元空间（MetaSpace）呢

  > 下图来自《深入理解Java虚拟机》第3版 
  >
  > ![img](img/ly-20241212141954289.png)
...
- Class文件中除了有**类的版本**、**字段**、**方法**、**接口**等描述信息外，还有用于存放编译器期生成的**各种字面量（Literal）**和**符号引用（Symbolic Reference）**的**常量池表（注意，这里的常量池表，说的是刚刚编译后的那个class文件字节码代表的含义）**  
  ...
  > ![ly-20241212141954440](img/ly-20241212141954440.png)
...
- **常量池表**会在类加载后存放到**方法区**的**运行时常量池**中   
  ![ly-20241212141954600](img/ly-20241212141954600.png)
...
![ly-20241212141954749](img/ly-20241212141954749.png)

![ly-20241212141954891](img/ly-20241212141954891.png)
...
![ly-20241212141955038](img/ly-20241212141955038.png)

---

![ly-20241212141955180](img/ly-20241212141955180.png)

---

![ly-20241212141955326](img/ly-20241212141955326.png)
...
1. 句柄
   ...
   【也就是多了一层】![对象的访问定位-使用句柄](img/ly-20241212141955467.png)

2. 直接指针
   如果使用**直接指针**访问，**reference 中存储的直接就是对象的地址**。
   ![对象的访问定位-直接指针](img/ly-20241212141955616.png)
