...
- 下面给出的图是否是堆（通过定义）
  ...
  1，2是。
  3不是。
  ![ly-20241212141845610](img/ly-20241212141845610.png)
...
2. 从底向上，如果**父节点**比**该元素小**，则该节点和父节点交换（其实就是一棵树有3个（最多）节点，与树上最大的节点比较）
   直到无法交换（已经与根节点比较过）
   ![ly-20241212141846433](img/ly-20241212141846433.png)
...
- **自底向上堆化**
  ...
  2. 比较**根节点（当前节点）**的**左子节点**和**右子节点**，也就是下标为 2 ，3 的数组元素，将较大的元素填充到**根节点（下标为1）（当前遍历节点）**的位置
     ![ly-20241212141846962](img/ly-20241212141846962.png)

     > 此时又空出一个位置了，老规矩，谁有能力谁上

  3. 一直循环比较**空出位置**的**左右子节点**，并将较大者移至空位，直到堆的最底部
     ![ly-20241212141847134](img/ly-20241212141847134.png)
...
- 自顶向下堆化
  ...
  1. 第一件事情，就是把石头抬起来，从海面扔下去。这个石头就是**堆的最后一个元素**，我们**将最后一个元素移动到堆顶**。
     ![ly-20241212141847315](img/ly-20241212141847315.png)
  2. 将这个石头沉入海底，不停的与**左右子节点**的值进行比较，和**较大的子节点**交换位置，直到**无法交换位置**
     ![ly-20241212141847486](img/ly-20241212141847486.png)
  3. 结果
     ![ly-20241212141847663](img/ly-20241212141847663.png)
...
- 也就是对**所有非叶子结点**进行自顶向下
  ...
  ![ly-20241212141847840](img/ly-20241212141847840.png)
  这个构建堆操作的时间复杂度为O(n)
  ![ly-20241212141848011](img/ly-20241212141848011.png)

- 首先要了解哪些是非叶节点，**最后一个结点的父节点及它（这个父节点）之前的元素**，都是非叶节点。也就是说，如果**节点个数为n**，那么我们需要对**n/2到1的节点进行自顶向下（沉底）堆化**

- 如图
  ![ly-20241212141848182](img/ly-20241212141848182.png)

  1. 首先将初始的无序数组抽象为一棵树，图中的节点个数为6，所以4，5，6是叶子节点，1，2，3节点为非叶节点  
  2. 对1，2，3节点进行**自顶向下（沉底）**堆化，注意，顺序是从后往前堆化，从3号开始，一直到1号节点。
     - 3号节点堆化结果  
       ![ly-20241212141848360](img/ly-20241212141848360.png)
     - 2号节点堆化结果
       ![ly-20241212141848537](img/ly-20241212141848537.png)
     - 1号节点堆化结果
       ![ly-20241212141848711](img/ly-20241212141848711.png)
...
- 步骤
  1. 取出第一个元素并堆化
     ![ly-20241212141848892](img/ly-20241212141848892.png)
  2. 取出第2个元素并堆化
     ![ly-20241212141849064](img/ly-20241212141849064.png)
  3. 取出第3个元素并堆化
     ![](img/ly-20241212141849233.png)
  4. 取出第4个元素并堆化
     ![ly-20241212141849412](img/ly-20241212141849412.png)
  5. 取出第5个元素并堆化
     ![ly-20241212141849590](img/ly-20241212141849590.png)
  6. 取出第6个元素并堆化
     ![ly-20241212141849797](img/ly-20241212141849797.png)
  7. 排序完成