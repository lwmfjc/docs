...
GPIOA 和 GPIOC 是 STM32 芯片里面两组“管脚控制模块”，A、C代表不同的GPIO端口（Port）。 ~~STM32 将 GPIO 引脚按照端口（Port）划分，每个端口最多包含16个引脚，用 A/B/C 等字母区分。~~  
...
没有开启时钟的话，GPIOA外设模块无法正常工作，寄存器配置和读写不会生效。
...
它是 STM32里面负责：*开关外设时钟，配置系统时钟，复位外设*的模块。 ~~RCC就是 STM32 的「总电闸」，RCC类似STM32的*电源管理中心和时钟管理中心*，负责系统*时钟配置*、*外设时钟开启关闭*以及*外设复位*。~~   
...
STM32==*把外设分到不同总线*==上 ~~因为不同总线具有不同速度和时钟控制。~~ ：
...
50MHz表示GPIO输出驱动能力等级，代表该引脚==能够支持的最高==输出速度。实际翻转频率==还受到CPU执行速度、代码效率和总线速度影响==。   
...
| 模式           | 核心意思             | 谁控制引脚                  | 输出特点                   | 常见用途                      |
| ------------ | ---------------- | ---------------------- | ---------------------- | ------------------------- |
| **通用推挽输出00** | 普通GPIO输出         | **你的代码控制**（ODR寄存器）     | 可以主动输出高电平和低电平（3.3V/0V） | LED、电机控制、普通控制信号           |
| **通用开漏输出01** | 普通GPIO输出，但只能“拉低” | **你的代码控制**（ODR寄存器）     | 只能主动输出0，高电平靠外部上拉电阻提供   | 需要开漏特性的普通GPIO，例如电平转换、共享线路 |
| **复用推挽输出10** | GPIO交给片上外设控制     | **硬件外设控制**（USART/TIM等） | 外设可以主动输出高低电平           | 串口TX、PWM输出                |
| **复用开漏输出11** | GPIO交给片上外设控制     | **硬件外设控制**（I2C等）       | 外设只能拉低，高电平靠外部上拉        | I2C通信                     |
 
 ~~推挽是一种输出电路结构；通用/复用描述的是控制来源。~~  
...
你现在用的就是它。 ~~推挽是一种输出电路结构；通用/复用描述的是控制来源。~~   

------

通用推挽和复用推挽的区别，不在这个电路，而在于：谁来控制这两个开关。  
...
英文：Push-Pull。为什么叫推挽？因为里面有两个晶体管。  
...
如上，只要有一个设备把公共总线拉低，那么这根总线上的所有设备读到的都是低电平。 ~~因为设备内部的下拉开关把 SDA 总线连接到了 GND，使 SDA 的电压被拉低到接近 0V。~~ 

只有所有设备都释放（不开启下拉开关），总线才会被上拉电阻拉成高电平。 ~~因为没有设备把 SDA 拉向 GND，上拉电阻把 SDA 电压提升到 3.3V，形成高电平。此时几乎没有电流流动。~~ 
...
上拉电阻限制电流，同时将SDA电位提升到3.3V。(电阻的作用是避免了短路，即拉高电压，但限制电流。)
...
ODR寄存器，里面有16位，即：ODR每一位对应一个GPIO引脚的输出状态  
